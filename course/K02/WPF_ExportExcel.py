...
class MyWindow(Window):
    ...
    def btnExport(self, sender, event):
        with forms.WarningBar(title='Export Data To Excel:'):
            path = pyBIM3DMLab.get_path_file()
            with open(path,"wb") as file:
                ...
                with ProgressBar(title='My Process... ({value} of {max_value})') as pb:
                    for counter in range(0, max_value):
                        ...
                        item = sheets[counter]
                        writer.writerow([item.count, item.sheet_number, item.sheet_name, item.amount_view, item.designer, item.drawnBy, item.checked, item.date])
                os.startfile(path)
        self.Close()
